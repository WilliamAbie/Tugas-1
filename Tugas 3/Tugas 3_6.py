class MenuMinuman:
    def __init__(self,nama,deskripsi,harga):
        self.nama       = nama
        self.deskripsi  = deskripsi
        self.harga      = harga

mnm1 = MenuMinuman("Jus Jambu","Jus Jambu Merah tanpa gula",8500)
mnm2 = MenuMinuman("Jus Alpukat Ori","Jus Alpukat dengan tambahan air gula merah",15000)
mnm3 = MenuMinuman("Jus Alpukat Xtra Milk","Jus Alpukat dengan campuran susu coklat dan taburan kepingan choco",15000)
mnm4 = MenuMinuman("Red & Smooth","Smoothie pisang susu dengan stroberi",17500)

# Object yang ditambahkan
mnm5 = MenuMinuman("Gilus Mix","Kopi Hitam New Zealand dengan tambahan Gula Aren",12000)
mnm6 = MenuMinuman("Milk Tea Boba's","Minuman penyegar gerah yang mengandung campuran teh,susu, dan boba",15000)

pilihan_minuman = [mnm1,mnm2,mnm3,mnm4,mnm5,mnm6]
print("Daftar Menu Healthy & Fruits")
for mn in pilihan_minuman:
    t = "{} harga Rp. {}, {}".format(mn.nama,mn.harga,mn.deskripsi)
    print(t)